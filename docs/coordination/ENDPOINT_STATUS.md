...
| GET /transactions/{id} | 3 | **2026-07-24** | **live** | Promoted on test/live/detail-contract.live.spec.tsx — FUNDED demo fixture through useTransaction, floor visible to its owning supplier as a 3-dp string (bank-side absence: floor.live.spec.tsx). transaction detail · body varies by audience: supplier/platform get `minimumAcceptableAmount`, a bank never does (INV-8) |
...
| POST …/{id}/listing | 5 | **2026-07-24** | **live (2026-07-24, listing.live.spec.tsx)** | built. 201. Show the fee BEFORE confirming (ZM-FEE-007) — it is incurred at activation whether or not financing succeeds. Returns both deadlines · UI: supplier listing-activation screen, requires a real ELIGIBLE transaction |
| GET …/{id}/listing-current * | 5 | **2026-07-24** | **live (2026-07-24, listing.live.spec.tsx)** | v3.1.0 · built. 404 when never listed. offerCount present for supplier/platform only · UI: v3.1.0 · listing-activation + offer comparison screens |
| GET /listings/{id} | 5 | — | mock | built. Same role split as listing-current |
| GET /listings/{id}/offers | 5 | **2026-07-24** | **live** | Promoted on test/live/acceptance.live.spec.tsx — real ACTIVE offer through useListingOffers inside the real SessionProvider. built. **role-split**: supplier gets every ACTIVE offer in full; a bank gets its OWN offer or an empty array — never a competitor, never a count (INV-11) · UI: role-split · offer comparison screen (supplier), own-offer check (bank) |
...
| GET /marketplace/listings/{id} * | 5 | **2026-07-24** | **live (2026-07-24, listing.live.spec.tsx — floor-free body, INV-8)** | v3.1.0 · built. **403** (not 404) when the bank was evaluated and excluded — it is entitled to know it was. No floor, no offerCount, no competitors · UI: v3.1.0 · bank underwriting view, incl. the Phase 4 risk components |
...
| POST /offers/{id}/accept | 6 | **2026-07-24** | **live** | Promoted on test/live/acceptance.live.spec.tsx — accepted through useOfferAcceptance; a second call on the same attempt replayed the identical snapshot (id, hash, capturedAt): one lock observed twice (INV-1/INV-4). DEMO-CRITICAL · built. **200, no request body.** Returns the AcceptedOfferSnapshot, not the offer. A replay returns the SAME snapshot with 200 — safe to retry. Second offer on a locked transaction is 409 TRANSACTION_ALREADY_LOCKED. Irreversible: a database trigger refuses to clear the lock · UI: DEMO-CRITICAL · acceptance modal, atomic-in-memory + idempotency-key replay-safe (ZM-SEL-001..008) |
...
| POST/GET …/{id}/contract | 6 | **2026-07-24** | **GET live / POST mock** | GET promoted on test/live/detail-contract.live.spec.tsx — the seeded signed contract through useContract, past PENDING_SIGNATURES; POST (generation) has no live screen coverage yet. built. POST is **201**; 409 if already generated (never regenerates a signable document); **422 with `details.findings[]`** when the ZM-CON-006 checks fail — render the whole list as a checklist. GET is visible to both parties only · UI: contract review screens (both portals) · generation gated by ZM-CON-006 |
...
| POST …/{id}/funding/mark-sent | 7 | **2026-07-23** | **live** (2026-07-24, funding.live.spec.tsx) | 200 + Settlement body. Idempotent by observation — a repeat returns the same settlement, no second journal. 409 only if never contracted. Does NOT set FUNDED (INV-10) |
| POST …/{id}/funding/otp | 7 | **2026-07-23** | **live** (2026-07-24, funding.live.spec.tsx) | **201**. Plaintext returned once, to the bank only. 429 past `otp_max_resends` (3) |
...
| GET/POST /admin/commission-tiers | 9 | **2026-07-24** | **GET live / POST mock** (admin.live.spec.tsx — tiers table, 3-dp money; POST has no screen yet) | |
| GET /admin/audit-logs | 9 | **2026-07-24** | **live (2026-07-24, admin.live.spec.tsx — audit screen, server-side entity filter)** | |
| GET /admin/relisting-requests · approve | 9 | — | mock | partly v3.1.0 |
| POST /demo/time-travel | 9 | **2026-07-24** | **live** | Promoted on test/live/time-machine.live.spec.tsx — TimeMachineControl against the real endpoint: disarmed 404 surfaced as 'not armed'; armed zero-day jump renders the server's offset/effective date. The forward-jump proof is phase9-demo integration's, deliberately not the screen test's (it must never move the shared clock). DEMO-CRITICAL |
