...
export const endpointStatus: EndpointStatusEntry[] = [
  ...
  { method: "GET", path: "/buyers/search", phase: 3, status: "live", notes: "wizard step 1 · candidate list, never pre-selected (ZM-BUY-009). Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - candidates from the real registry mirror rendered by BuyerStep; no candidate pre-selected (ZM-BUY-009)" },
  { method: "POST", path: "/buyers/resolve", phase: 3, status: "live", notes: "wizard step 1 · 409 BUYER_BLOCKED renders the blocked-buyer reason. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - resolved the demo buyer as the chooser does" },
  { method: "GET", path: "/buyers/{id}", phase: 3, status: "mock", notes: "handler exists; screens read the buyer off the transaction instead" },
  { method: "POST", path: "/documents/upload-url", phase: 3, status: "live", notes: "wizard steps 2 and 3 · byte upload is skipped under MSW by design. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - signed-URL flow; the byte PUT is the browser own" },
  ...
  { method: "GET", path: "/documents/{id}/extraction", phase: 3, status: "live", notes: "wizard step 2 · OCR/QR pre-fill and mismatch table. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - genuine OCR over uploaded pixels; finalizes the hash" },
  ...
  { method: "POST", path: "/transactions", phase: 3, status: "live", notes: "wizard entry — creates the draft. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - the wizard entry - real draft created" },
  ...
  { method: "PUT", path: "/transactions/{id}/invoice", phase: 3, status: "live", notes: "wizard step 2. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - PDF-true fields; outstanding recomputed server-side" },
  { method: "PUT", path: "/transactions/{id}/buyer", phase: 3, status: "live", notes: "wizard step 1. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - buyer linked" },
  { method: "PUT", path: "/transactions/{id}/minimum-amount", phase: 3, status: "live", notes: "wizard step 4. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - the private floor set" },
  { method: "POST", path: "/transactions/{id}/declarations", phase: 3, status: "live", notes: "wizard step 5 · template version \"1.0\", server-validated against a catalogue (Q-13 resolved). Promoted 2026-07-24 on test/live/wizard.live.spec.tsx — all eight affirmed" },
  { method: "POST", path: "/transactions/{id}/submit", phase: 3, status: "live", notes: "wizard step 6 · 409 duplicate → blocked screen, reference is details.reviewReference (Q-11 resolved). Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - real automated checks end at ELIGIBLE" },
  { method: "GET", path: "/transactions/{id}/verification", phase: 3, status: "live", notes: "verification panel on the transaction detail. Promoted 2026-07-24 on test/live/wizard.live.spec.tsx - the verification panel reads a real run" },
  ...
  { method: "POST", path: "/transactions/{id}/relist-request", phase: 9, status: "mock", notes: "Served: writes a REQUESTED row, one open per transaction (409 on a second). phase9-admin" },
  { method: "POST", path: "/transactions/{id}/cancel", phase: 9, status: "mock", notes: "Served: cancellable up to OPEN_FOR_OFFERS (offers closed with it), 409 once an offer is accepted, CANCELLED never a delete. phase9-admin" },
